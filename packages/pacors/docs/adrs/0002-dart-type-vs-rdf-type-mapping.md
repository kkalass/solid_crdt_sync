...
The pacors library organizes configuration around Dart types (Note, Category, etc.) but the underlying Solid ecosystem operates on RDF type IRIs (schema:NoteDigitalDocument, schema:CreativeWork, etc.). This creates a potential impedance mismatch with several architectural implications.
...
We will enforce a 1:1 mapping between Dart types and RDF type IRIs in the pacors system.
...
Comprehensive validation is implemented using a validation context pattern in the `pacors_core` package:
...
See `packages/pacors_core/lib/src/config/validation.dart` for implementation details.
