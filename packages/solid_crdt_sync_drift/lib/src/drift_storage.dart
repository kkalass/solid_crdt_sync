...
class DriftStorage implements LocalStorage {
  ...
  Future<void> deleteResource(String resourceIri) async {
    ...
    await _db.transaction(() async {
      ...
      await (_db.delete(_db.rdfTriples)
        ..where((triple) => triple.documentIri.equals(resourceIri))).go();
      
      // Delete metadata
      await (_db.delete(_db.crdtMetadata)
        ..where((meta) => meta.resourceIri.equals(resourceIri))).go();
      
      // Delete index entries
      await (_db.delete(_db.indexEntries)
        ..where((entry) => entry.resourceIri.equals(resourceIri))).go();
      
      // Delete document
      await (_db.delete(_db.rdfDocuments)
        ..where((doc) => doc.documentIri.equals(resourceIri))).go();
    });
  }
  ...
  void _ensureInitialized() {
    if (!_initialized) {
      throw StateError('DriftStorage not initialized. Call initialize() first.');
    }
  }
